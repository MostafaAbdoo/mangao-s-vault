...
Third Part : [[Nmap Advanced Port Scans]]
...
Nmap can detect the Operating System (OS) based on its behaviour and any telltale signs in its responses. OS detection can be enabled using `-O`; this is an _uppercase O_ as in OS. In this example, we ran `nmap -sS -O 10.10.111.35` on the AttackBox. Nmap detected the OS to be Linux 3.X, and then it guessed further that it was running kernel 3.13.
```shell-session
sudo nmap -sS -O 10.10.111.35
...
In the following example, we executed `nmap -sS --traceroute 10.10.111.35` on the AttackBox. We can see that there are no routers/hops between the two as they are connected directly.
```shell-session
sudo nmap -sS --traceroute 10.10.111.35
...
Let’s consider a benign script, `http-date`, which we guess would retrieve the http server date and time, and this is indeed confirmed in its description: “Gets the date from HTTP-like services. Also, it prints how much the date differs from local time…” On the AttackBox, we execute `sudo nmap -sS -n --script "http-date" 10.10.36.70` as shown in the console below.
```shell-session
sudo nmap -sS -n --script "http-date" 10.10.36.70

Starting Nmap 7.60 ( https://nmap.org ) at 2021-09-10 08:04 BST
Nmap scan report for 10.10.36.70
Host is up (0.0011s latency).
Not shown: 994 closed ports
PORT    STATE SERVICE
22/tcp  open  ssh
...
Pentester Terminal
           ...
           `pentester@TryHackMe$ grep http MACHINE_IP_scan.nmap  80/tcp  open  http    nginx 1.6.2 OS and Service detection performed. Please report any incorrect results at https://nmap.org/submit/ .`
        

Pentester Terminal

           `pentester@TryHackMe$ grep http MACHINE_IP_scan.gnmap  Host: 10.10.36.70	Ports: 22/open/tcp//ssh//OpenSSH 6.7p1 Debian 5+deb8u8 (protocol 2.0)/, 25/open/tcp//smtp//Postfix smtpd/, 80/open/tcp//http//nginx 1.6.2/, 110/open/tcp//pop3//Dovecot pop3d/, 111/open/tcp//rpcbind//2-4 (RPC #100000)/, 143/open/tcp//imap//Dovecot imapd/	Ignored State: closed (994)	OS: Linux 3.13	Seq Index: 257	IP ID Seq: All zeros`
